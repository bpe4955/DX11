...
Sky::Sky(std::shared_ptr<Mesh> _mesh, Microsoft::WRL::ComPtr<ID3D11SamplerState> _sampleState, 
	...
	mesh(_mesh)
{
	...
	skyTexture = CreateCubemap(device, context,
		(filePath + std::wstring(L"right.png")).c_str(),
		(filePath + std::wstring(L"left.png")).c_str(),
		(filePath + std::wstring(L"up.png")).c_str(),
		(filePath + std::wstring(L"down.png")).c_str(),
		(filePath + std::wstring(L"front.png")).c_str(),
		(filePath + std::wstring(L"back.png")).c_str()
	);
	...
	stenDesc.DepthFunc = D3D11_COMPARISON_LESS_EQUAL;
	device->CreateDepthStencilState(&stenDesc, &stencilState);

	ps = std::make_shared<SimplePixelShader>(device, context, FixPath(L"SkyPS.cso").c_str());
	vs = std::make_shared<SimpleVertexShader>(device, context, FixPath(L"SkyVS.cso").c_str());
}
Sky::~Sky() {}
	...
	Microsoft::WRL::ComPtr<ID3D11RasterizerState> _rasterizerState)
{
	...
	ps->SetSamplerState("Sampler", sampleState);
	ps->SetShaderResourceView("skyTexture", skyTexture);
